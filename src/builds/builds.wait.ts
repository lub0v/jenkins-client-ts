...
/**
 * Wait for build to complete and return build information
 *
...
export const wait = async (
  ...
  (await waitWhile<JenkinsBuild>(
    () => get(requests.clone({ params: { depth: 1 } }), path, buildNumber),
    r => !r.data || r.data.building,
    options,
  ))!;
